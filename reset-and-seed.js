 ...
 *
 * Нужен serviceAccountKey.json рядом с файлом (Firebase → Project Settings →
 * Service accounts → Generate new private key).
 *
 * ⚠️ ВНИМАНИЕ: это безвозвратно удаляет ВСЕ документы перечисленных коллекций.
 */
...
async function deleteCollection(name, batchSize = 300) {
  ...
  console.log(`  🗑  ${name}: удалено ${total}`);
}
...
async function run() {
  ...
  console.log(`\n🔥 Заливаю ${venues.length} заведений...`);
  const batch = db.batch();
  for (const x of venues) {
    const [from, to] = GRADIENT[x.category] || GRADIENT.cafe;
    batch.set(db.collection("venues").doc(x.id), {
      name: x.name,
      category: x.category,
      district: "Центр",
      address: x.address,
  ...
  console.log(`✅ Готово: ${venues.length} заведений в коллекции venues.`);
  process.exit(0);
}
