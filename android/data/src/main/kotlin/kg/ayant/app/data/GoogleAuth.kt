...
object GoogleAuth {
    ...
    suspend fun signIn(context: Context): AyantUser {
        ...
        // Вход поверх гостевого сеанса: сперва пробуем привязать (`link`) —
        // тогда гость сохраняет uid и всё, что успел накопить. Если такой
        // аккаунт уже есть, привязка невозможна — входим обычным способом.
        val guest = auth.currentUser?.takeIf { it.isAnonymous }
        val result = if (guest != null) {
            runCatching { guest.linkWithCredential(firebaseCred).await() }
                .getOrElse { auth.signInWithCredential(firebaseCred).await() }
        } else {
