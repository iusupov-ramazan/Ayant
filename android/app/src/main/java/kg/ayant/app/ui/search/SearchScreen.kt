    ...
    Scaffold(
        ...
        Column(Modifier.fillMaxSize().padding(padding)) {
            OutlinedTextField(
                ...
                placeholder = { Text("Заведение, категория или акция") },
            ...
            Row(
                ...
                FilterChip(selected = openNow, onClick = { openNow = !openNow }, label = { Text("Открыто") })
                FilterChip(selected = withDeals, onClick = { withDeals = !withDeals }, label = { Text("Со скидкой") })
            ...
            if (results.isEmpty()) {
                Box(Modifier.fillMaxSize(), contentAlignment = Alignment.TopCenter) {
                    Column(
                        ...
                        Text("Ничего не нашлось", fontSize = 18.sp, fontWeight = FontWeight.Bold, color = c.ink)
                        Text("Попробуй другое название или расширь расстояние.", fontSize = 14.sp, color = c.inkSoft)
                    }
                }
            } else {
                LazyColumn(
...
private fun RatingMenu(minRating: Int, onSelect: (Int) -> Unit) {
    ...
    Box {
        FilterChip(selected = minRating > 0, onClick = { open = true },
            label = { Text(if (minRating == 0) "Рейтинг" else "★ $minRating+") })
        DropdownMenu(expanded = open, onDismissRequest = { open = false }) {
            DropdownMenuItem(text = { Text("Любой рейтинг") }, onClick = { onSelect(0); open = false })
...
private fun DistanceMenu(maxDistance: Double?, onSelect: (Double?) -> Unit) {
    ...
    Box {
        FilterChip(selected = maxDistance != null, onClick = { open = true },
            label = { Text(if (maxDistance == null) "Расстояние" else "≤ ${if (maxDistance < 1) "${(maxDistance * 1000).toInt()} м" else "%.0f км".format(maxDistance)}") })
        DropdownMenu(expanded = open, onDismissRequest = { open = false }) {
            DropdownMenuItem(text = { Text("Любое расстояние") }, onClick = { onSelect(null); open = false })
...
private fun CategoryMenu(category: VenueCategory?, onSelect: (VenueCategory?) -> Unit) {
    ...
    Box {
        FilterChip(selected = category != null, onClick = { open = true },
            label = { Text(category?.rawValue ?: "Категория") })
        DropdownMenu(expanded = open, onDismissRequest = { open = false }) {
            DropdownMenuItem(text = { Text("Все категории") }, onClick = { onSelect(null); open = false })
