...
data class HostVenueDTO(
    ...
    // Бонусы САН (конфиг задаётся в админ-панели; читается для сканера)
    var pointsEnabled: Boolean = false,
    var pointsMode: String = "flat",
    var pointsFlat: Int = 0,
    var pointsBands: List<PointsBand> = emptyList(),
    var cashbackPercent: Double = 0.0,
    var pointsRewards: List<PointsReward> = emptyList(),
    var pointsExpiryMonths: Int = 6,
    var redeemMode: String = "staffScan",
    var earnCooldownMinutes: Int = 60,
) {
    val category: VenueCategory get() = VenueCategory(categoryRaw.ifEmpty { "Кафе" })
...
data class HostDealDTO(
    ...
    var imageURL: String = "",
    /** Условия акции, по строке на пункт. */
