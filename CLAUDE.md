...
1. **Баллы САН — per-venue points (the real loyalty product).** Business-funded, earned by scanning at *that* venue, spent on *that* venue's rewards. **Cannot be earned by games.**
   ...
   - **Files**: iOS `SAN/Bonus/VenuePointsViews.swift` (screens) + `AyantFeatures/Sources/AyantFeatures/PointsStore.swift` (state); Android `ui/vm/VenuePointsViewModel.kt` + `ui/bonus/VenuePointsScreens.kt`. Config rides in `HostVenueDTO`/`HostModels.kt` (read-only — host save does NOT write points fields, so it can't clobber admin-panel config).
