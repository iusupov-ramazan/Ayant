# Ayant — Telegram-бот поддержки (ИИ, бесплатно)

ИИ-помощник, который отвечает пользователям 24/7 на русском, опираясь на справку Ayant и FAQ. Работает в режиме long-polling — не нужен сервер с публичным адресом, бот можно запустить где угодно (свой ПК, бесплатный тариф Render/Railway/Fly.io).
...
2. Задай имя и username (напр. `AyantSupportBot`).
...
- Любое текстовое сообщение → ИИ-ответ (Groq), приправленный справкой Ayant и FAQ из `faq.js`.
